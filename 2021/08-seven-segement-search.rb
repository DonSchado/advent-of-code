...
segment_count_to_digit_mapping = {
  2 => 1,
  4 => 4,
  3 => 7,
  7 => 8,
...
input.map do |line|
  signal, digit_values = line.split('|').map(&:split)

  digit_values.each do |value|
    if segment_count_to_digit_mapping[value.size]
      digits << value
    end
  end
end
...
def find(signals, n)
  signals.select { |signal| signal.size == @segment_mapper[n].size }.map { |signal| signal.split('').sort }
end

output_values = []

input.each do |line|
  signals, digit_values = line.split('|').map(&:split)

  digit_1 = find(signals, 1).first
  digit_7 = find(signals, 7).first
  digit_4 = find(signals, 4).first
  digit_8 = find(signals, 8).first
  segment_1 = digit_7 - digit_1
  selector_2 = digit_8 - digit_4 - digit_7
  digit_2 = find(signals, 2).detect { |signal| (selector_2 - signal).empty? }
  segment_3 = (digit_1 - (digit_1 - digit_2))
  segment_6 = (digit_1 - digit_2)
  selector_5 = digit_4 - digit_1
  digit_5 = find(signals, 5).detect { |signal| (selector_5 - signal).empty? }
  digit_3 = (find(signals, 5) - [digit_2, digit_5]).first
  digit_9 = digit_5 + segment_3
  segment_5 = digit_8 - digit_9
  segment_2 = digit_8 - digit_3 - segment_5
  segment_4 = digit_4 - digit_1 - segment_2
  segment_7 = digit_8 - segment_1 - segment_2 - segment_3 - segment_4 - segment_5 - segment_6
  digit_6 = segment_1 + segment_2 + segment_4 + segment_5 + segment_6 + segment_7
  digit_0 = digit_8 - segment_4
  ...
  actual_number = digit_values.each_with_object([]) do |value, memo|
    memo << @digit_translation[value.split('').sort]
  end.join.to_i

  output_values << actual_number
end
