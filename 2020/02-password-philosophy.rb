...
def count_passwords(input, &validation)
  input.dup.reduce(0) do |count, line|
    policy, password = line.split(': ')
    min, max, char   = policy.scan(/\w+/)

    if validation.call(password, min, max, char)
      count += 1
    end
...
count = count_passwords(input) do |password, min, max, char|
