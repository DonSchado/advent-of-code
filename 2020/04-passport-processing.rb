...
# part 1
count = input.count do |passport_data|
  attributes = Hash[passport_data.split.map { |pair| pair.split(':') }]
  (attributes.keys - ['cid']).size == 7
end

puts 'Part 1: In your batch file, how many passports are valid?'
puts count
# => 192.
...
count = input.count do |passport_data|
  attributes = Hash[passport_data.split.map { |pair| pair.split(':') }]
  next unless (attributes.keys - ['cid']).size == 7
  next unless (1920..2002).cover?(attributes['byr'].to_i)  # byr (Birth Year)
  next unless (2010..2020).cover?(attributes['iyr'].to_i) # iyr (Issue Year)
  next unless (2020..2030).cover?(attributes['eyr'].to_i) # eyr (Expiration Year)
  if attributes['hgt'].include?('cm')
    next unless (150..193).cover?(attributes['hgt'].to_i)
  elsif attributes['hgt'].include?('in')
    next unless (59..76).cover?(attributes['hgt'].to_i) # hgt (Height)
  else
    next
  end
  next unless attributes['hcl'].match?(/\A#[a-f0-9]{6}\z/) # hcl (Hair Color)
  next unless %w[amb blu brn gry grn hzl oth].include?(attributes['ecl']) # ecl (Eye Color)
  next unless (attributes['pid'] && attributes['pid'].match?(/\A[0-9]{9}\z/)) # pid (Passport ID)

  true
end

puts 'Part 2: In your batch file, how many passports are valid?'
puts count
# => 101
