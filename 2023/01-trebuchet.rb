...
calibration = input.each_with_object([]) do |line, memo|
  values = line.scan(/\d/)

  memo << (values&.first.to_s + values&.last.to_s).to_i
end

p '1) What is the sum of all of the calibration values?'
p calibration.reduce(:+)
# => 55029
...
calibration = input.each_with_object([]) do |line, memo|
  tokenized = line.scan(/#{digits.keys.join('|')}|\d/)
  x, *, y   = tokenized.map { |token| (digits[token] || token) }.join.chars

  memo << (x + (y || x)).to_i
end

p '2) What is the sum of all of the [replaced] calibration values?'
p calibration.reduce(:+)
# => 55686
