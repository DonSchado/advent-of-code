games = DATA.readlines(chomp: true)

MAX_RED   = 12
MAX_GREEN = 13
MAX_BLUE  = 14

game_results = games.each_with_object({}) do |game, memo|
  id = game.match(/Game (?<id>\d+):/)[:id].to_i

  possible = game.split(';').all? do |round|
    red   = (round.match(/(?<red>\d+) red/) || {})[:red].to_i
    green = (round.match(/(?<green>\d+) green/) || {})[:green].to_i
    blue  = (round.match(/(?<blue>\d+) blue/) || {})[:blue].to_i
...
p "What is the sum of the IDs of the possible games?"
p game_results.select { |id, possible| possible == true }.keys.sum
# => 2156


power_games = games.each_with_object({}) do |game, memo|
  id = game.match(/Game (?<id>\d+):/)[:id].to_i

  cubes = game.split(';').each_with_object({ red: 0, green: 0, blue: 0 }) do |round, counts|
    red   = (round.match(/(?<red>\d+) red/) || {})[:red].to_i
    green = (round.match(/(?<green>\d+) green/) || {})[:green].to_i
    blue  = (round.match(/(?<blue>\d+) blue/) || {})[:blue].to_i

    counts[:red]   = [red,   counts[:red]].max
    counts[:green] = [green, counts[:green]].max
    counts[:blue]  = [blue,  counts[:blue]].max
  end

  memo[id] = cubes
end

p "What is the sum of the power of all the games?"
p power_games.map { |game, cubes| cubes.values.reduce(:*) }.sum
# => 66909
