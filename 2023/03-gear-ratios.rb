schematic = DATA.readlines(chomp: true)
...
schematic.each_with_index do |line, y|
  next unless line.match?(SYMBOL)

  line.chars.each_with_index.select { |cell, x| cell.match?(SYMBOL) }.each do |sym, x|
    symbols_map[[y, x]] = sym
  end
end
...
def relevant_neighbours_for(schematic, y, x)
  min_y, max_y = 0, schematic.size
  min_x, max_x = 0, schematic[y].size

  n  = [(y-1).clamp(min_y, max_y), x]
  nw = [(y-1).clamp(min_y, max_y), (x-1).clamp(min_x, max_x)]
  ne = [(y-1).clamp(min_y, max_y), (x+1).clamp(min_x, max_x)]
  e  = [y, (x+1).clamp(min_x, max_x)]
  s  = [(y+1).clamp(min_y, max_y), x]
  sw = [(y+1).clamp(min_y, max_y), (x-1).clamp(min_x, max_x)]
  se = [(y+1).clamp(min_y, max_y), (x+1).clamp(min_x, max_x)]
  w  = [y, (x-1).clamp(min_x, max_x)]

  [
    nw, n, ne,
    w,     e,
    sw, s, se
  ].select { |ny, nx| schematic[ny][nx].match?(/\d/) }
end

def scan_number(schematic, coord_y, coord_x)
  char   = schematic[coord_y][coord_x]
  number = [char]
  ...
  # left
  until left < 0 || schematic[coord_y][left] == '.' || schematic[coord_y][left]&.match?(SYMBOL)
    char = schematic[coord_y][left]
    number.prepend(char)
    left -= 1
  end

  # right
  until right > schematic[coord_y].size || schematic[coord_y][right] == '.' || schematic[coord_y][right]&.match?(SYMBOL)
    char = schematic[coord_y][right]
    number.append(char)
    right += 1
  end
...
# p symbols_map.values.uniq.each_with_object({}) {|s, h| h[s] = s.match?(SYMBOL) }

symbols_map.each do |(y, x), sym|
  scans = relevant_neighbours_for(schematic, y, x).map do |neighbor_y, neighbor_x|
    scan_number(schematic, neighbor_y, neighbor_x)
  end

  if sym == GEAR && scans.uniq.size == 2
    gear_ratio = scans.uniq.reduce(:*)
    gear_ratios << gear_ratio
  end

  part_numbers.concat(scans.uniq)
end

# p part_numbers
p part_numbers: part_numbers.sum
# => 539433

p gear_ratios: gear_ratios.sum
# => 75847567
