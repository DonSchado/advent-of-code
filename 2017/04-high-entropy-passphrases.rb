...
passphrases = DATA.read.chomp.split("\n").map { |line| line.split(' ') }

# part 1
puts 'How many passphrases are valid? (without duplicate words)'
puts passphrases.count { |words| words == words.uniq }

# part 2
puts 'How many passphrases are valid? (without anagrams)'
sorted = ->(word) { word.chars.sort.join }
puts passphrases.count { |words| words.map(&sorted) == words.map(&sorted).uniq }
