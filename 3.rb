...
wire_path_1 = wire_to_path_points(wire_1)
wire_path_2 = wire_to_path_points(wire_2)

intersections = (wire_path_1 & wire_path_2).map do |intersection|
  next if intersection == CENTRAL_PORT

  manhattan_distance(CENTRAL_PORT, intersection)
end.compact

puts "Manhattan distance from the central port to the closest intersection:"
puts intersections.min


# part 2
puts "Fewest combined steps the wires must take to reach an intersection:"

def visits(wire)
  x, y    = CENTRAL_PORT
  counter = (1..Float::INFINITY).lazy

  wire.each_with_object({}) do |move, visits|
    direction, steps = move.scan(/[URDL]|\d+/)

    1.upto(steps.to_i).each do
      case direction
      when 'U' then y += 1
      when 'R' then x += 1
      when 'D' then y -= 1
      ...
      steps_until_here = counter.next
      visits[[x, y]] ||= steps_until_here
    end
  end
end

steps_1 = visits(wire_1)
steps_2 = visits(wire_2)

puts (steps_1.keys & steps_2.keys).map { |point| steps_1[point] + steps_2[point] }.min
