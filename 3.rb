...
def grid_coords(wire)
  wire.each_with_object([CENTRAL_PORT]) do |move, path|
    path.concat(to_grid_coords(move, path.last))
  end
end

def to_grid_coords(move, point = CENTRAL_PORT)
...
intersections = (grid_coords(wire_1) & grid_coords(wire_2)).map do |intersection|
