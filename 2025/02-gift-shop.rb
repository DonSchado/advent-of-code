input = DATA.readlines(chomp: true).flat_map { _1.split(',') }

ranges = input.map { Range.new(*_1.split('-')) }

invalids = []
ranges.each do |range|
  range.each do |id|
    if id.size.even?
      left, right = id.split(/(?<=^.{#{id.size/2}})/)
      invalids << id if left == right
    end
  end
end

puts 'part 1'
puts invalids.sum(&:to_i)
# => 54641809925
...
ranges.each do |range|
  range.each do |id|
    (1..id.size/2).each do |length|
      pattern = id[0, length]
      if pattern * (id.size / length) == id
        invalids << id
        break
      end
    end
