...
def command(instruction)
  instruction.match(/turn\soff|turn\son|toggle/).to_s.sub(' ', '_').to_sym
end

def args(instruction)
  instruction.scan(/\d+/).map(&:to_i).each_slice(2).to_a
end
...
instructions.dup.each do |instruction|
  case command(instruction)
  when :toggle
    change(*args(instruction)) { |x, y| @lights[y][x] = @lights[y][x].zero? ? 1 : 0 }
  when :turn_off
    change(*args(instruction)) { |x, y| @lights[y][x] = 0 }
  when :turn_on
    change(*args(instruction)) { |x, y| @lights[y][x] = 1 }
  end
end

# part 1
puts 'lights on:'
puts @lights.flatten.count { |light| light >= 1 }
# 400410
...
instructions.dup.each do |instruction|
  case command(instruction)
  when :toggle
    change(*args(instruction)) { |x, y| @lights[y][x] += 2 }
  when :turn_off
    change(*args(instruction)) { |x, y| @lights[y][x] -= 1 if @lights[y][x] > 0 }
  when :turn_on
    change(*args(instruction)) { |x, y| @lights[y][x] += 1 }
  end
end
