...
def checksum(name)
  name
    ...
    .join
    .delete('123456789')[0..4]
end

def parse(encrypted)
  name, checksum = encrypted.split(/-\d+/)
  id             = encrypted.scan(/\d+/).first

  yield(id.to_i, name, checksum.delete('[]'))
end

def rotate(name, times)
  alphabet = ('a'..'z').to_a.join
  rotation = alphabet.chars.rotate(times).join

  name.tr(alphabet, rotation).gsub('-', ' ')
end

input = DATA.read.chomp.split("\n")

# puts checksum('aaaaa-bbb-z-y-x') == 'abxyz'
# puts checksum('a-b-c-d-e-f-g-h') == 'abcde'
# puts checksum('not-a-real-room') == 'oarel'
# puts checksum('totally-real-room') != 'decoy'
# puts checksum('aczupnetwp-mfyyj-opalcexpye') == 'peyac'
# puts checksum('ajvyjprwp-kdwwh-bjunb') == 'jwbpa'
# puts checksum('jvuzbtly-nyhkl-yhiipa-zavyhnl') == 'yhlai'

# part 1
puts 'What is the sum of the sector IDs of the real rooms?'
real_rooms = input.each_with_object([]) do |encrypted, rooms|
  parse(encrypted) do |id, name, checksum|
    rooms << id if checksum(name) == checksum
  end
end
puts real_rooms.sum # => 361724

# part 2
# puts rotate('qzmt-zixmtkozy-ivhz', 343) == 'very encrypted name'
puts 'What is the sector ID of the room where North Pole objects are stored?'
input.each do |encrypted|
  parse(encrypted) do |id, name, checksum|
    if checksum(name) == checksum
      rotation = rotate(name, id)
      if rotation.include?('north')
        puts "#{id}: #{rotation}"
        exit
      end
    end
